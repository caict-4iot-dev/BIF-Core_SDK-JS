    ...
    async batchContractInvoke (request = {}) {
        ...
        for (var key in operations) {
            if (Object.hasOwnProperty.call(operations, key)) {
                ...
                var contractInvokeParam = {
                    to: contractAddress,
                    amount: amount,
                    input: operations[key].input
                }
                arr.push(contractInvokeParam)
            }
        }
        ...
        arr = arr.reduce(function (item, next) {
            obj[next.to] ? '' : obj[next.to] = true && item.push(next)
            return item
        }, [])
        // console.log(arr)
        ...
        transaction.addBatchContractOperation('payCoin', contractInvoke)
        let query = new Query(options)
